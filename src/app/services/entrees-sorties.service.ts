...
export class EntreesSortiesService {
  ...
  public saveEntree(formValue:any) {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');
    return this.http.post<any>(this.config_service.apiHostAddress+"/entree",
    {
      produit: {idProduit: parseInt(formValue.produit, 10)},
      dateEntree: this.constructDate(),
      qteEntree: formValue.qteEntree,
      raisonEntree: formValue.raisonEntree
    },
    {headers});
  }
